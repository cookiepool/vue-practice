...
<div class="all_wrap">
   ...
   <div class="head_tips_wrap">
       <div class="bac"></div>
       <div class="head_tips_title flex_all_center">
    ...
    <div class="auditing_wrap">
        ...
        <div class="input_wrap flex">
            <p>资料审核</p><input v-model="phone" placeholder="输入任务注册账号/手机号" />
        </div>
        <div class="input_wrap flex" v-for="(item,index) in des" :key="index"  >
            <p>{{item.key}}</p><input type="text" v-model="fields[index]" :placeholder="item.key" />
        </div>        
    </div>
    <!-- 上传截图凭证 -->
    ...
    <!-- 底部提交按钮 -->
    <div class="btn flex_all_center red" @click="submit">
        <p>立即申述</p>
    </div>
...
export default {
    ...
    methods:{
        ...
        submit(confirm){
            var _this = this;
            if(confirm !== true) {
                this.$MessageBox.confirm("每个任务只有一次申述的机会,确认提交?").then(action => {
                    if(action == "confirm"){
                        _this.submit(true);
                    }
                }).catch(action =>{
            ...
            api.heignTaskSubmitComplaint(params).then(res => {
                if(res.data.status == true){
                    this.$loadding.close();
                    this.$MessageBox.alert("您的申述已提交完成");
                    this.$router.push({path: '/heighList'});
...
.tips_item span{
    margin: 0 0.8571428571428571rem 0 0.5714285714285714rem;
}
.auditing_wrap{
